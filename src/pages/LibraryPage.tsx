...
export function LibraryPage({ revision, onSettings, notify }: { revision: number; onSettings: () => void; notify: (toast: ToastState) => void }) {
  ...
  async function importPaths(candidatePaths: string[]) {
    ...
    setImporting(true);
    setProgress({ completed: 0, total: paths.length, currentFile: fileName(paths[0]), done: false });
    let importedCount = 0;
    let duplicateCount = 0;
    try {
      for (let index = 0; index < paths.length; index += 1) {
        const path = paths[index];
        setProgress({ completed: index, total: paths.length, currentFile: fileName(path), done: false });
        const added = await api.importVideos([path]);
        importedCount += added.length;
        if (!added.length) duplicateCount += 1;
        setProjects((current) => [...added, ...current]);
        setProgress({ completed: index + 1, total: paths.length, currentFile: fileName(path), done: index + 1 === paths.length });
      }
      ...
      setProgress(null);
      notify({ kind: "error", message: `已导入 ${importedCount} 个文件，随后导入失败：${String(error)}` });
    } finally {
  ...
  return (
    <div className={`app-shell ${dragActive ? "drag-import-active" : ""}`}>
      ...
      {progress && <div className="modal-scrim import-progress-scrim"><section className="import-progress-modal" role="dialog" aria-modal="true" aria-labelledby="import-progress-title"><div className={`import-progress-icon ${progress.done ? "done" : ""}`}>{progress.done ? <CheckCircle size={30} weight="fill" /> : <SpinnerGap size={30} className="spin" />}</div><p className="eyebrow">视频导入</p><h2 id="import-progress-title">{progress.done ? "导入完成" : "正在创建视频项目"}</h2><div className="import-current-file"><FileVideo size={18} /><span title={progress.currentFile}>{progress.currentFile}</span></div><div className="download-progress"><div className="progress-track"><span style={{ transform: `scaleX(${progress.total ? progress.completed / progress.total : 0})` }} /></div><div className="progress-copy"><span>{progress.completed} / {progress.total} 个文件</span><strong>{Math.round((progress.completed / progress.total) * 100)}%</strong></div></div><p className="import-progress-note">导入期间请勿关闭应用</p></section></div>}
